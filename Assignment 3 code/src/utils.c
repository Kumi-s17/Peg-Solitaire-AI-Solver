...
void initialize(state_t *board, int8_t layout) {
	...
	//if layout is greater than 8 then make layout = 0
	if( layout > NUM_LAYOUTS - 1) layout = 0;
	
	//Always start the cursor with the (4,4) th element but unselected
	board->cursor.x = 4;
	board->cursor.y = 4;
	board->selected = false;
...
void play_solution(){
	printf("solution size is %d", solution_size);
	for(int i=0; i <= solution_size; i++){
